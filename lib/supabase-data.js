import { makeId, normalizeConsumption } from "./review-store";
...
function isMissingConsumptionColumn(error) {
  const message = String(error?.message || "").toLowerCase();
  return message.includes("consumption") && (message.includes("column") || message.includes("schema cache"));
}

function withoutConsumption(payload) {
  const { consumption, ...rest } = payload;
  return rest;
}
...
export async function upsertRemoteReview(supabase, review, profile) {
  ...
  if (review.id) {
    ...
    if (error && isMissingConsumptionColumn(error)) {
      const retry = await supabase
        .from("event_reviews")
        .update(withoutConsumption(payload))
        .eq("id", review.id)
        .select()
        .single();
      data = retry.data ? { ...retry.data, __consumptionPendingMigration: true } : retry.data;
      error = retry.error;
    }
