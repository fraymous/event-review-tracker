import { makeId, normalizeConsumption, normalizeGuestCount } from "./review-store";
...
export function dbReviewToUi(row, signedUrls = {}) {
  ...
  return {
    ...
    consumption: normalizeConsumption(row.consumption),
...
function uiReviewToDb(review, profile) {
  return {
    ...
    client_contact: review.clientContact || "",
    event_date: review.eventDate,
    guest_count: guestCountToDb(review.guestCount),
    venue: review.venue,
    ...
    consumption: normalizeConsumption(review.consumption),
    ...
    follow_up_notes: review.followUpNotes || "",
    updated_at: new Date().toISOString(),
  };
}

function isMissingReviewColumn(error, column) {
  const message = String(error?.message || "").toLowerCase();
  return message.includes(column) && (message.includes("column") || message.includes("schema cache"));
}

function withoutMissingOptionalColumns(payload, error) {
  const omitted = new Set();
  if (isMissingReviewColumn(error, "consumption")) omitted.add("consumption");
  if (isMissingReviewColumn(error, "guest_count")) omitted.add("guest_count");
  if (omitted.size === 0) return null;
  return Object.fromEntries(Object.entries(payload).filter(([key]) => !omitted.has(key)));
}

export async function signedUrlsForAttachments(supabase, rows) {
...
export async function upsertRemoteReview(supabase, review, profile) {
  ...
  if (review.id) {
    ...
    const fallbackPayload = error ? withoutMissingOptionalColumns(payload, error) : null;
    if (error && fallbackPayload) {
      const retry = await supabase
        ...
        .select()
        .single();
      data = retry.data ? { ...retry.data, __optionalColumnsPendingMigration: true } : retry.data;
      error = retry.error;
    }
