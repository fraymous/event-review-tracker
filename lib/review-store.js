...
export function formatGuestCount(value) {
...
export function buildCsv(reviews) {
  ...
  const rows = reviews.map((review) => [
    review.clientName,
    review.eventDate,
    formatGuestCount(review.guestCount),
  ...
  return [...(reviews || [])]
    .filter((review) => {
      ...
      const matchesAttachments =
        normalized.attachments === "All" ||
        (normalized.attachments === "Yes" && attachmentCount > 0) ||
        (normalized.attachments === "No" && attachmentCount === 0);
      const consumptionApplies = getConsumptionApplies(review.consumption);
      const matchesConsumption =
        normalized.consumption === "All" ||
        (normalized.consumption === "Yes" && consumptionApplies) ||
        (normalized.consumption === "No" && !consumptionApplies);
      ...
      return matchesTerm && matchesStatus && matchesTag && matchesManager && matchesAttachments && matchesConsumption && matchesDue && matchesDateFrom && matchesDateTo;
    })
