...
function ReviewDetail({ review, role, links, onCopy = () => {}, onCreateShare, onDelete = () => {}, onDuplicate = () => {}, onEdit, onOpen = () => {}, onPrint, onSetStatus = () => {} }) {
  ...
  const activeLinkCount = links.filter(isShareActive).length;
  const attachmentCount = (review.attachments || []).length;
  ...
  return (
    <div className="detail-layout">
      ...
      <section className="content-band"><div className="section-heading"><h3>Shared Access</h3><span>{activeLinkCount} active</span></div><div className="share-mini-list">{links.length === 0 && <p className="small-muted">No links created.</p>}{links.slice(0, 3).map((link) => <div className="share-mini" key={link.id}><div><span>{isShareActive(link) ? "Active" : "Inactive"}</span><em>Expires {formatDate(link.expiresAt.slice(0, 10))}</em></div>{role === "manager" && <div className="row-actions"><button className="icon-button" onClick={() => onOpen(link.token)} title="Open" type="button"><ExternalLink size={16} /></button><button className="icon-button" onClick={() => onCopy(link.token)} title="Copy" type="button"><Copy size={16} /></button></div>}</div>)}</div></section>
