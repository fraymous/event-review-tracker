      ...
      <section className="workspace">
        <header className="topbar">
          ...
          <div className="topbar-actions">
            ...
            {remoteMode && session && profile && <button aria-label="Refresh cloud data" className="icon-button" disabled={remoteLoading} onClick={refreshCloudData} title="Refresh cloud data" type="button"><RefreshCw size={18} /></button>}
            <button aria-label="Print current view" className="icon-button" onClick={() => window.print()} title="Print" type="button"><Printer size={18} /></button>
            {remoteMode && <button aria-label="Sign out" className="icon-button" onClick={signOut} title="Sign out" type="button"><LogOut size={18} /></button>}
          </div>
        </header>
  ...
  return (
    <div className="dialog-backdrop" role="presentation">
      <section aria-labelledby="confirm-title" aria-modal="true" className={`confirm-dialog ${tone}`} role="dialog">
        <button aria-label="Cancel dialog" className="dialog-close" onClick={onCancel} title="Cancel" type="button"><X size={16} /></button>
    ...
    return (
      <div className="share-row" key={link.id}>
        ...
        <div className="row-actions">
          <button aria-label={`Open ${name} link`} className="icon-button" onClick={() => onOpen(link.token)} title="Open" type="button"><ExternalLink size={16} /></button>
          <button aria-label={`Copy ${name} link`} className="icon-button" onClick={() => onCopy(link.token)} title="Copy" type="button"><Copy size={16} /></button>
          {role === "manager" && !link.revokedAt && <button aria-label={`Revoke ${name} link`} className="icon-button" onClick={() => onRevoke(link.id)} title="Revoke" type="button"><Trash2 size={16} /></button>}
        </div>
      </div>
    );
  }
